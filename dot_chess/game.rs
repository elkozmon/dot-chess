...
impl Game {
    ...
    pub fn make_move(&self, ply: Ply) -> Result<Self> {
        ...
        let (board, state, zhash, halfmove_clock) = self.make_pseudo_legal_move(ply)?;
    ...
    fn make_pseudo_legal_move(
        &self,
        ply: Ply,
    ) -> Result<(Board, State, ZobristHash, HalfmoveClock)> {
        // Assert sides turn
        let (side, piece) = self
            .board
            .piece_at(ply.from())
        ...
        match piece {
            Piece::Pawn => {
                ...
                if let (Rank::_2, Rank::_4) | (Rank::_7, Rank::_5) = (rank_from, rank_to) {
                    ...
                    // Early return
                    return Ok((board_new, state_new, zhash_new, halfmove_clock_new));
                }
        ...
        Ok((board_new, state_new, zhash_new, halfmove_clock_new))
    }
}
...
mod tests {
    ...
    fn make_pseudo_legal_move_pawn_c2_to_d2() {
        ...
        Game::new().make_pseudo_legal_move(ply).unwrap();
    }
}
